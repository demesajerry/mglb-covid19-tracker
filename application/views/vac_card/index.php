...
<!-- Content Header (Page header) -->
  <section class="content-header">
    <h1><i class="fa fa-id-card"></i> Vaccination Card</h1>
    <ol class="breadcrumb">
      <li class="active">Client Vaccination Card</li>
    </ol>
  </section>
...
<script type="text/javascript"> 
  ...
  $(document).ready(function(){
    ...
    table = $('#table').DataTable({ 
      ...
      select: true,
      "ajax": {
        "url": "<?php echo site_url('Vac_card/get_list')?>",
        "type": "POST",
        "data": function ( data ) {
    ...
    $('#table').on('click','.print', function(){
      ...
      $('#userid').val(userid);
      $('#name').html(`<h4><strong><u>${name}</u></strong></h4>`);
      $('#vac_card_title').html('Print Vaccination Card: '+fullname);
      $('.full_name').html(fullname);
      $('#vac_card_modal').modal('show');
    })
});
</script>
<?php  $this->load->view('vac_card/modal/index'); ?>
