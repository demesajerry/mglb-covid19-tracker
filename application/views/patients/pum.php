        ...
        <div class="row">
            <div class="col-xs-12">
                <div class="box">
                    ...
                    <h3 class="box-title">PUM</h3>
                        <div class="box-tools">
                            <button class="btn btn-info" id="add_patients" >
                                <i  class="fa fa-plus fa-1x"> </i> Add PUM Case
                            </button>
                        </div>
                    </div>
                <hr />
                    ...
                    <input type="hidden" name="" id="action">
                     ...
                     <table class="table table-bordered table-hover">
                        ...
                        <tr>
                          ...
                          <td>
                            <select name="current_location" id="current_location" class="select2 form-control" style="width: 100%">
...
$('#save_test').click(function(e){
  ...
  if ($("#form-add_test")[0].checkValidity()) {
    ...
    $('#confirm_title').html('<h3>Add Test Confirmation</h3>');
    $('#modal_message').html('<h4>If Result is POSITIVE. The record will automatically move to CONFIRMED List. </h4>');
...
$('#test_results_modal').on('click','.edit_test',function(e){
  ...
  if (test_date=='' || test_date=='0000-00-00') {
    ...
    $('#confirm_title2').html('<h3>Update Test Confirmation</h3>');
    $('#modal_message2').html('<h4>If Result is POSITIVE. The record will automatically move to CONFIRMED List. </h4>');
...
$('#submit_add').click(function(e){
  ...
  e.preventDefault();
    ...
    if ($("#form1")[0].checkValidity()) {
      ...
      success: function(data){ 
        if(action == 'add'){
          $(`<div class='alert alert-info messageBox'>${data.name} has been added.</div>`).insertBefore( "#viewresult" );
        }else{
          $(`<div class='alert alert-info messageBox'>${data.name} has been edited.</div>`).insertBefore( "#viewresult" );
        }
...
createdRow: function (row, data, index) {
        ...
        var qend = new Date(data['qdate']);
        console.log(qend);
        if (qend <= today && data['qdate']!='') {
            $(row).addClass("text-highlight warning");
        }
    }
});  
});
