...
<!-- Content Header (Page header) -->
  <div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
      <h1 class="h3 mb-0 text-gray-800"><i class="fa fa-folder"></i> Disabled Client List</h1>
    </div>
    <!-- Content Row -->
...
<script type="text/javascript"> 
  ...
  $('#table').on('click','#reset_password', function(){
    ...
    table = $('#table').DataTable({ 
      ...
      select: true,
      "ajax": {
        "url": "<?php echo site_url('Vac_list/ajax_list_disabled')?>",
        "type": "POST",
        "data": function ( data ) {
  ...
  $('#showpass').click(function(){
    ...
    $('#table').on('click','.enable', function(e){
      ...
      var client_id = this.getAttribute('client_id');
      var name = this.getAttribute('name');
      $('#eclient_id').val(client_id);
      $('#enable_display').html(`<h2>Are you sure you want to ENABLE ${name}</h2>`);
    ...
    $('#confirm_enable').click(function(){
      var client_id = $('#eclient_id').val();
      $.ajax({
        url : "<?php echo site_url('vac_list/enable')?>/",
        type: "POST",
        data:{client_id:client_id},
        dataType: "JSON",
          success: function(data){
            //if success reload ajax table 
            toastr.success('Client has been enabled!');
